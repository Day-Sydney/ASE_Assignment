...
#
### Comment from Sydney Day for Assignment: ###
# style-blend function/option removed, due to trying to
# simplify/speed up interaction with GUI
# attempts at rearranging conv/relu layers resulted in 
# segmentation faults. Do not recommend.
            ...
            for i in range(iterations):
                ...
                if (checkpoint_iterations and i % checkpoint_iterations == 0) or last_step:
                    ...
                    if preserve_colors and preserve_colors == True:
                        preserving_colors(original_image,styled_image,content,img_out)
                else:
                    img_out = None
...
def preserving_colors(original_image,styled_image,content,img_out):
        original_image = np.clip(content, 0, 255)
        styled_image = np.clip(img_out, 0, 255)

        # Luminosity transfer steps:
        # 1. Convert stylized RGB->greyscale accoriding to Rec.601 luma (0.299, 0.587, 0.114)
        # 2. Convert stylized greyscale into YUV (YCbCr)
        # 3. Convert original image into YUV (YCbCr)
        # 4. Recombine (stylizedYUV.Y, originalYUV.U, originalYUV.V)
        # 5. Convert recombined image from YUV back to RGB

        # 1
        styled_greyscale = rgb2grey(styled_image)
        styled_greyscale_rgb = grey2rgb(styled_greyscale)

        # 2
        styled_greyscale_yuv = np.array(Image.fromarray(styled_greyscale_rgb.astype(np.uint8)).convert('YCbCr'))
        ...
        combined_yuv[..., 0] = styled_greyscale_yuv[..., 0]
